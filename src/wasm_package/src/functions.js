...
const format_litecoin_message_len = (msg, flags) => {
    if (flags & C.BITCOIN_MESSAGE_FLAG_HASH) {
        return C.SHA256_LEN
    } else {
        const msg_len = msg.length
        return 26 + msg_len + (msg_len < 253 ? 1 : 3)
    }
}

export const format_litecoin_message = wrap('wally_format_litecoin_message', [T.Bytes, T.Int32, T.DestPtrVarLen(T.Bytes, format_litecoin_message_len, true)]);
...
export const psbt_kernel_get_mweb_presign_stealth_key = wrap('wally_psbt_kernel_get_mweb_presign_stealth_key', [T.OpaqueRef, T.DestPtrVarLen(T.Bytes, C.WALLY_PSBT_MWEB_PRESIGN_SCALAR_LEN)]);
export const psbt_kernel_set_mweb_presign_stealth_key = wrap('wally_psbt_kernel_set_mweb_presign_stealth_key', [T.OpaqueRef, T.Bytes]);
...
export const psbt_output_get_mweb_presign_sender_key = wrap('wally_psbt_output_get_mweb_presign_sender_key', [T.OpaqueRef, T.DestPtrVarLen(T.Bytes, C.WALLY_PSBT_MWEB_PRESIGN_SCALAR_LEN)]);
...
export const psbt_output_set_mweb_presign_sender_key = wrap('wally_psbt_output_set_mweb_presign_sender_key', [T.OpaqueRef, T.Bytes]);
...
export const psbt_strip_mweb_presign_fields = wrap('wally_psbt_strip_mweb_presign_fields', [T.OpaqueRef]);
