...
struct wally_psbt_output {
    ...
    * TODO: replace this with actual TR representation when TR implemented */
    ...
    struct wally_map taproot_leaf_paths;
#ifdef BUILD_MWEB
    /* MWEB output fields. First-class struct slots so MWEB outputs
     * round-trip through wally_psbt_output by direct field access,
     * matching the MWEB input pattern on wally_psbt_input above.
     * Presence is tracked by mweb_output_keyset bits (MWEB_OUT_*). */
    unsigned char mweb_stealth_address[66];   /* 0x90: A(33) || B(33) (host-supplied) */
    unsigned char mweb_commit[33];            /* 0x91: C_out (device-built) */
    uint8_t mweb_features;                    /* 0x92: feature bits */
    unsigned char mweb_sender_pubkey[33];     /* 0x93: K_s (device-built) */
    unsigned char mweb_output_pubkey[33];     /* 0x94: K_o (device-built) */
    /* 0x95: K_e(33) || view_tag(1) || masked_value(8 LE) || masked_nonce(16 BE).
     * Stored as one blob so the byte layout matches the on-wire
     * MwebOutputMessage standard-fields section exactly. */
    unsigned char mweb_standard_fields[58];
    unsigned char *mweb_range_proof;          /* 0x96: heap-owned, typically 675 B */
    size_t mweb_range_proof_len;
    unsigned char mweb_signature[64];         /* 0x97: per-output Schnorr (device-built) */
    unsigned char *mweb_extra_data;           /* 0x98: heap-owned, variable length */
    size_t mweb_extra_data_len;
